...
If you use a proxy, install SkinsRestorer on the proxy and all backend servers.

## How Proxy Mode works

In Proxy Mode, the proxy controls SkinsRestorer for the complete network.
...
The `detection` option selects the mode:

- `AUTO` (default): Detects whether the server is behind a proxy
- `ENABLED`: Enables Proxy Mode
- `DISABLED`: Disables Proxy Mode

### Backend API access

The `api` option lets backend plugins use the SkinsRestorer API. Its default value is `true`.

This option needs MySQL. The proxy and backend servers must use the same database.

### Disable Proxy Mode

Without Proxy Mode, each backend server manages skins independently. Skin data does not automatically stay synchronized.
...
<Callout type="warning">
  Use the same `config.yml` file on the proxy and all backend servers.
</Callout>
...
<Callout type="warning">
  If you disable Proxy Mode, each server manages skins independently. Skin data does not synchronize between servers.
</Callout>
