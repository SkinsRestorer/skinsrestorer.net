...
SkinsRestorer uses its own API endpoints to cache and distribute skin data efficiently:
...
These endpoints act as a cache layer between your server and Mojang's API.
This way, we don't have to ask Mojang for the skin data every time a player joins a server, which helps avoid rate limits.
The cache is also helpful when Mojang's servers are slow or down, as we can still serve skin data from the cache.
